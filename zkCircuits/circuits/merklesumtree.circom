...
// Computes the next level of the merkle sum tree, namely poseidon hash and the next sum given the current hash and sum, 
// the sibling hash and sum, and the path index
...
template NextMerkleSumTreeLevel(){
    ...
    //according to the index of the path, mux is selecting which leaf is left and which is right to perform the hash
    ...
    //start the Addup() component

    component addUp = Addup();

    addUp.in[0] <== mux.out[1];
    addUp.in[1] <== mux.out[3];

    //finally output the result of the poseidon hash and sum which is sum (leftSum + rightSum)
